...
class ChiSquareNarratives(object):
    ...
    def __init__(self, df_helper, df_chisquare_result, spark, df_context, data_frame, story_narrative,result_setter,scriptWeight=None, analysisName=None):
        ...
        self.new_effect_size,self.signi_dict,self.actual_cols,self.sort_list = self.feat_imp_threshold(target_dimension)
       ...
       #preprocess_df = DataPreprocessingAutoML_obj.data_frame
        ...
        if FeatureEngineeringAutoML_obj.dimension_cols != []:
            ...
            for col in FeatureEngineeringAutoML_obj.dimension_cols:
                if self._pandas_flag:
                    if FeatureEngineeringAutoML_obj.data_frame[col].nunique() < 20:
            ...
            if label_list != []:
                if self._pandas_flag:
                    for column_name in label_list:
                        clean_df[column_name] = LabelEncoder().fit_transform(clean_df[column_name])
        ...
        sort_list = list((k for k, v in sort_dict.items() if v > 0))
        if self._pandas_flag:
        ...
        for key,value in si_var_dict.items():
            ...
            if value < 0.8:
                ...
                si_var_thresh[key] = value
                break
        return feat_imp_dict,si_var_thresh,actual_cols,sort_list
    ...
    def _generate_narratives(self):
        ...
        for target_dimension in list(self._df_chisquare_result.keys()):
            ...
            analysed_sort= set(self.sort_list) & set(analysed_variables)
            # List of significant var out of analyzed var.
            significant_variables_chi = [dim for dim in analysed_sort]
            effect_sizes_chi = [target_chisquare_result[dim].get_effect_size() for dim in significant_variables_chi]
            # effect_size_dict = dict(list(zip(significant_variables,effect_sizes)))
            effect_size_dict = self.new_effect_size
            significant_variables_fe = list(self.signi_dict.keys())
            #effect_sizes = list(self.signi_dict.values())
            significant_variables = [y for (x,y) in sorted(zip(effect_sizes_chi,significant_variables_chi) ,reverse=True)]
            ...
            data_dict = {
                          ...
                          'num_significant_variables' : len(significant_variables_fe),
                          'significant_variables' : significant_variables_fe,
            ...
            if self._appid=='2' and num_significant_variables>5:
                ...
                if self._nColsToUse != None:
                    if len(significant_variables) > 8:
                        self._nColsToUse = len(significant_variables)
